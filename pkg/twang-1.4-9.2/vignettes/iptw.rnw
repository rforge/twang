...
%\VignetteIndexEntry{Propensity scores for repeated treatments: A tutorial for the iptw function}
...
treatement instead of another, researchers are often
...
may be impacted by prior treatments. Consequently, simply controling
...
propensity score-based method, observations are weighted by the
inverse of the estimated probability of 
receiving the observed sequence of treatments using nonparametric
models to estimate the propensity scores (Griffin et al.,
2014). Accordingly, we refer 
to the function in \texttt{twang} that performs this weighting as
\texttt{iptw}, for {\em inverse probability of treatment weighting}. 

The \texttt{iptw} methods and syntax build directly on the \texttt{ps}
...
For the sake of easy dissemination, we simulated data to demonstrate
the functionality. For repeated measures data, one can either imagine
a ``wide'' dataset, with one row per subject, or a ``long'' dataset
with a row for each subject/time point combination. Our artificial
data include time-invariant characteristics \texttt{gender}, and \texttt{age} at time of
...
period, which concludes the study.  In the ``wide'' version of the
dataset, we have the \texttt{outcome}, baseline and intermediate
measures, \texttt{use0}, \texttt{use1}, and \texttt{use2}. The
treatment indicators are, in chronological order, \texttt{tx1},
\texttt{tx2}, and \texttt{tx3}.  
...
the outcomes are split from the covariates, and are available at
\texttt{outcome}. Similarly, the covariates and treatment
...
To begin, we will work with the ``wide'' version of the model, which
are available after loadin the twang package:
...
Next, we can examine propensity score overlap: 
...
TWANG figures, we can specify \texttt{color = FALSE} to produce black and
figures. 
...
@ 
 ...
 We can estimate the treatment effect using these unstablized weights
...
We can then calculate the 95\% confidence interval using the
unstablized weights as
<<>>=
confint(svyglm(outcome ~ nTx, sv1unstab))
@ 
...
As before, we can then estimate the treatment effect
<<>>=
confint(svyglm(outcome ~ nTx, sv1stab))
@ 
