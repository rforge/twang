...
plot.mnps <- function(x,plots="optimize", pairwiseMax = TRUE, figureRows = NULL, color = TRUE, subset = NULL, treatments = NULL, ...)
{
   ...
   if(!pairwiseMax | plots == "optimize" | plots == 1){
	...
	plotTab$sig <- as.numeric(plotTab$p < 0.05)
   	...
   	if(pairwiseMax) nPlotsTot <- 1
   	else if(length(treatments) == 2) nPlotsTot <- 1
   	...
   	else if(length(treatments) == 2) ptNames[[1]] <- c(treatments)
